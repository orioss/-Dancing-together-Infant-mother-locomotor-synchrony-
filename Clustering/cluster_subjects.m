%function [cl]=cluster_subjects(data,percent)
data = subject_time_patterns;
percent = 10
mdist_square = squareform(pdist(data));
mdist = [];
for i=1:size(mdist_square,1)
    for j=i:size(mdist_square,2)
      mdist = [mdist; i j mdist_square(i,j)];  
    end
end
%figure;
%imagesc(mdist_square);
